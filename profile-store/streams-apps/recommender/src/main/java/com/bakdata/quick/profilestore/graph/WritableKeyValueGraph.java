...
import com.bakdata.quick.profilestore.avro.AdjacencyList;
...
public class WritableKeyValueGraph extends KeyValueGraph implements WriteableBipartiteGraph {
    ...
    public void addEdge(final long leftNodeId, final long rightNodeId) {
        updateIndex(leftNodeId, rightNodeId, this.leftIndex);
        updateIndex(rightNodeId, leftNodeId, this.rightIndex);
    }
     ...
     */
    private static void updateIndex(final long keyId, final long valueId,
            final KeyValueStore<Long, AdjacencyList> index) {
        final AdjacencyList adjacencyList = getAdjacencyList(keyId, valueId, index);
        index.put(keyId, adjacencyList);
    }

    private static AdjacencyList getAdjacencyList(final Long leftId, final Long rightId,
            final KeyValueStore<Long, AdjacencyList> index) {
